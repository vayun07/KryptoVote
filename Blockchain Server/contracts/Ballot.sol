...
contract Ballot {
	...
	function createElection(bytes32 election, uint duration) public {
		...
		votingdeadline = block.timestamp + (duration * 1 minutes); //'block.timestamp' has replace 'now'
	}
	 ...
	 */
	function Ballot1(bytes32 candid, bytes32 party, bytes32 symbol) public returns(bytes32 c) {
	 ...
	 * name of a candidate
	 */
	function addCandidate(string memory candidateName) public returns(bytes32 Name) {
	 ...
	 *boolean value
	 */
	function vote(bytes32 candidateName, address ethadd) public returns(bool check) {
		//Voter storage sender = voters[msg.sender];
		Voter storage sender = voters[ethadd];
		if (sender.voted) {
			return true;
		}
		else {
	...
	*/
	 ...
	 * @return {uint} winningCandidatePosition
	 * the highest number of votes
	 *
	 */

	function winningCandidate() public view returns(uint winningCandidatePosition) {
		uint winningVoteCount = 0;
		uint p = 0;
		for (uint i = 0; i < candidates.length; p++) {
			if (candidates[p].voteCount > winningVoteCount) {
				winningVoteCount = candidates[i].voteCount;
				winningCandidatePosition = i;
			}
		}
	}
	/*
	 *This is a readonly function which returns the name of the winnner candidate
	 *@return {string} nameOfWinner
	 *winner name
	 */
	function winnerName() public view returns(string memory nameOfWinner) {
		nameOfWinner = bytes32ToString(candidates[winningCandidate()].name);
	}
