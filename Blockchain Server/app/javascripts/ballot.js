...
window.voteForCandidate = function(name) {
  ...
  try {
    ...
    var username = name;
              ...
              var account = web3.eth.accounts[0];//web3.eth.coinbase;
              //var unlock = web3.personal.unlockAccount(account, "Krypto123456");
        //console.log("Account unlocked: " + unlock);
      ...
      contractInstance.getVoted.call(ethaddress).then(function(voted){
        ...
        if(res == true) 
        {
              ...
              console.log("checkpoint: " + checkpoint);
                ...
                var meth = web3.sha3('vote(bytes32)'); //0xcc9ab267
                    ...
                    var gasEstimate = 2 * (web3.eth.estimateGas({t}));
                    //console.log("Gas estimate value: "+gasEstimate);
                    var privateKeyCoinbaseBuffer= new Buffer(privateKey, 'hex');
					
					contractInstance.vote(web3.toHex(candidateName), ethaddress, {gas: gasEstimate, from:account}).then(function(a){
